...
MODELNAME_STRINGS = ['opt-decoder-', 'opt-model-', 'bertmodel_', 'gpt2model_', 'optdecoder_', \
                     'mobilevitmodel_', 'optmodel_', 'resnet101_', 'vitmodel_', 'qwen2model_', 'gptossmodel_', 'resnet50_']

LANGUAGE_MODELS = ['optmodel', 'bertmodel', 'gpt2model', 'optdecoder', 'qwen2model', 'gptossmodel']
VISION_MODELS = ['vitmodel', 'mobilevitmodel', 'resnet101', 'resnet50']
...
def get_compute_kernel_portion(ncu_path, prefix):
    ...
    # if 'mobilevit' in prefix:
    #     for idx, row in ncu_df.iterrows():
    #         kernel_name = row['kernel_name']
    #         for s in kernel_strings:
    #             if s in kernel_name.lower():
    #                 break
    #             if s == kernel_strings[-1]:
    #                 print(kernel_name)
...
def main():
    ...
    parser.add_argument('--measurement_path', type=str, nargs='*', required=True)
    ...
    # check if there are multiple measurement_path
    measurement_ncu_path = args.measurement_path[0]
    if len(args.measurement_path) > 1:
        assert(args.measurement_kernel_breakdown==False, 'Kernel breakdown with NCU measurement is only supported for a single path now')
        
    # Measurements
    measured = None
    for p in args.measurement_path:
        nvml_measured_path = os.path.join(p, 'nvml/nvml_parsed.csv')
        temp = pd.read_csv(nvml_measured_path)
        if measured is None:
            measured = temp
        else:
            measured = pd.concat([measured, temp])

    ncu_measured_path = os.path.join(measurement_ncu_path, 'ncu')
    ...
    for config in estimated_configurations:
        ...
        if config_params['model'] in LANGUAGE_MODELS:
            filename_prefix = '{}_{}_p{}_b{}_s{}_mode{}'.format(config_params['model'], \
            ...
            filename_prefix = '{}_{}_p{}_b{}'.format(config_params['model'], \
                                                     config_params['config'], \
                                                     args.precision if args.precision is not None else config_params['prec'], \
                                                     config_params['batch'])
        ...
        try:
            ...
            # print("Measurement result for this config was not found: ", config)
            continue
        ...
        for e in estimators_used:
            estimated_match = estimated[(estimated['workload'] == config) & (estimated['estimator'] == e)].iloc[0]
